...
\nt{
	Chi-squared test
	\begin{align*}
		H_0 & : \text{ the population follows law M }        \\
		H_1 & : \text{ the population does not follow law M}
	\end{align*}

	Test statistic
	\[
		Y = \sum_{i=1}^{k} \frac{(n_{i} - n_{p_i})^2}{n_{p_i}} \sim {\chi^2}_{k-1}
		.\]
...
\begin{tikzpicture}
	% Figure that shows linear regression on a sample data set with error bars
	\begin{axis}[
			axis lines = left,
			xlabel = $x$,
			ylabel = {$y$},
